...
var (
	...
	root = &cobra.Command{
		Use:   "webhook-manager",
		Short: "Callback Manager, for all your callback needs!",
	}
)

func init() {
	cobra.OnInitialize(initConfig)
	root.PersistentFlags().StringVar(&configFile, "config", "", "config file (default is ${HOME}/.webhook-manager.yaml")
}
...
func initConfig() {
	...
	if configFile != "" {
		...
		viper.SetConfigName(".webhook-manager.yaml")
	}

	// enable environment vars
	viper.SetEnvPrefix("CALLBACK_MANAGER")
